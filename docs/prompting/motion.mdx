...
> …the card lands by 2s, then holds motionless to the end.

✅ **With**

> …the card lands by 2s, then nothing ever fully stops: the cluster breathes between 0.95× and 1.05× on a 1.6s cycle, a wide glow sweeps behind it end to end and wraps off-frame so it never pauses, the eyebrow pulses, and the accent rule's sheen crosses its full length three times.
...
*Left `WITHOUT`, right `WITH` — the left is bit-identical frame to frame after it lands; on the right, worst-case adjacent-frame PSNR is 28 dB — every frame differs.*
