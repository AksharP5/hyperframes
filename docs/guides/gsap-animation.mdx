...
<Warning>
  This is one of the most common mistakes in HyperFrames. If your video cuts off early, the timeline is too short. See [Troubleshooting: Composition Duration Shorter Than Video](/guides/troubleshooting) for more details.
</Warning>
...
<Warning>
  Don't animate `width`, `height`, `top`, or `left` directly on `<video>` elements — this can cause the browser to stop rendering frames. Wrap the video in a `<div>` and animate the wrapper instead. See [Troubleshooting](/guides/troubleshooting) for a detailed explanation.
</Warning>
