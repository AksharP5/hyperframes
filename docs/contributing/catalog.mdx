...
<Info>
**Quick version** — Fork the repo. Write one HTML file with a paused GSAP timeline. Add `registry-item.json`. Run `npx hyperframes lint` and `npx hyperframes check`. Publish with `npx hyperframes publish`. Open a PR.
</Info>
